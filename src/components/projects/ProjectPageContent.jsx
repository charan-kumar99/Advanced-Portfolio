...
import { X, Box, Copy, Check, Maximize2, ArrowLeft, Users, Layers, LayoutGrid, ChevronRight, ChevronLeft, Github, ExternalLink, Zap, Terminal, Sparkles, Clock } from 'lucide-react';
...
const TerminalBlock = ({ title, code }) => {
...
export function ProjectPageContent({ project, isLowPowerMode }) {
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4 border-y border-black/20 dark:border-border/40 py-8">
                    <div className="flex flex-col gap-2">
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold flex items-center gap-2">
                            <Box className="w-3 h-3" /> {t('metadata.role')}
                        </span>
