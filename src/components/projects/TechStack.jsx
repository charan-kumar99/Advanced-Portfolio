...
import { Wrench, ArrowDown, RefreshCw } from 'lucide-react';
...
export function TechStack({ techStack, tools, isLowPowerMode }) {
    ...
    useEffect(() => {
        ...
        const createBody = (name, isTool, index, total) => {
        ...
        setBodies(newBodies);
    }, [techStack, tools, isLowPowerMode, resolvedTheme]);
                        ...
                        <div className="absolute inset-0 flex items-center justify-center p-8">
                            <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-4 sm:gap-6 max-w-5xl mx-auto">
                                {[...techStack, ...(tools || [])].map((item, idx) => {
                                    const icon = portfolioData.techStack.find((t) => t.name.toLowerCase() === item.toLowerCase())?.icon ||
                                        portfolioData.tools.find((t) => t.name.toLowerCase() === item.toLowerCase())?.icon;
