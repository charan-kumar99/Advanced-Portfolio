...
import { ChevronDown, Trophy, Navigation, Briefcase, Rocket, ImageIcon, FileText, MessageCircle } from 'lucide-react';
import { cn } from '@/lib/utils';

function GridSnake({ theme }) {
...
function MegaBoxBig({ href, icon: Icon, title, desc, theme, pathname }) {
...
function MegaBoxSmall({ href, icon: Icon, title, desc, theme, pathname }) {
    const isActive = pathname === href || pathname?.startsWith(`${href}/`);
...
function SidebarLink({ href, icon: Icon, title, desc, theme, pathname }) {
    const isChat = href === '#';
    ...
    const className = cn(
        ...
        isChat ? "cursor-not-allowed hover:scale-[1.02] hover:-translate-x-1 hover:shadow-xl" : "cursor-pointer hover:scale-[1.02] hover:-translate-x-1 hover:shadow-xl"
    );
...
export default function CardNav({
    ...
    const allHrefs = ['/projects', '/experience', '/skills', '/achievements', '/gallery', '/resume'];
