...
function FeaturedCard({ project, onClick, index, isLowPowerMode }) {
...
function CompactCard({ project, onClick, index }) {
    const [isHovered, setIsHovered] = useState(false);
    const isOngoing = project.status === 'ongoing';
...
const getIconKey = (name) => {
    ...
    return Object.keys(Icons).find(k => lower.includes(k.toLowerCase())) || 'unknown';
};
...
export default function ProjectsPage() {
    ...
    const categories = [
        ...
        { id: 'Full Stack', label: 'Full Stack & .NET', icon: Database },
    ...
    const filters = [{ key: 'all', label: t('filters.all') }, { key: 'ongoing', label: t('filters.ongoing') }, { key: 'completed', label: t('filters.completed') }];
