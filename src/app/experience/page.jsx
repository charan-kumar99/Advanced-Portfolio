...
function ExperienceHighlightSection({ type, isLowPowerMode }) {
...
function FloatingShape({ className, gradient, delay = 0, isLowPowerMode }) {
    return (
        <motion.div
...
function ExperienceTabSlider({ isLowPowerMode }) {
    const contentRef = useRef(null);
    const [activeTab, setActiveTab] = useState(1);
    const [selectedCategory, setSelectedCategory] = useState(null);

    const tabs = [
        { id: 'education', label: 'Education', description: 'Building strong foundations through academic excellence at MIT Jaipur and Mangalore University.' },
...
function CollapsibleExperienceCard({ exp, idx, isLowPowerMode }) {
...
const LinkPreviewCard = ({ url, title, id, logo }) => {
...
function TimelineGallery({ images, id, title, externalLink, logo }) {
    const [isExpanded, setIsExpanded] = useState(false);
    ...
    const allImages = verifiedImages.map((src, i) => ({ src, index: i, type: 'image' }));
    const validImages = allImages.filter(img => !failedImages.has(img.index));

    const galleryItems = [
        ...validImages,
        ...(externalLink ? [{ type: 'link', src: externalLink, index: validImages.length }] : [])
    ];
...
function ExperienceTimeline({ isLowPowerMode }) {
                    ...
                    <div key={exp.id} className="relative pl-8 border-l-2 border-neutral-200 dark:border-neutral-800">
                        ...
                        <div className="mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                                ...
                                <p className="text-lg font-medium text-primary flex items-center gap-2 flex-wrap">
                                    <span>{exp.company}</span>
                                    {exp.location && (
                                        <>
                                            <span className="w-1.5 h-1.5 rounded-full bg-neutral-300 dark:bg-neutral-700 shrink-0" />
                                            <span className="text-sm text-neutral-500 dark:text-neutral-400 font-normal">{exp.location}</span>
                                        </>
                                    )}
                                </p>
                            </div>
