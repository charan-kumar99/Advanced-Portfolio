...
function buildSystemPrompt(locale = 'en') {
    const { personal, projects, experience, education, skills, achievements, softSkills, tools } = portfolioData;
    ...
    const skillList = (skills ?? [])
        .map((s) => `${s.name} (${s.level ?? s.proficiency ?? ''})`)
        .join(', ');
...
- Languages: ${(personal.languages ?? []).map((l) => `${l.name} (${l.level})`).join(', ')}
- GitHub: ${(personal.socialLinks ?? []).find((s) => s.platform === 'GitHub')?.url ?? ''}
- LinkedIn: ${(personal.socialLinks ?? []).find((s) => s.platform === 'LinkedIn')?.url ?? ''}
...
## Education
${eduList || 'MCA from MIT Jaipur, BCA from Mangalore University.'}

## Technical Skills
${skillList || '.NET Core, C#, Python, React, Microservices.'}

## Soft Skills
${softSkillList || 'Leadership, Communication, Problem Solving.'}

## Tools & Technologies
${toolList || 'VS Code, Docker, GitHub, PostgreSQL, Oracle.'}
